...
// ── POST-Requests (öffentlich + admin) ──
if ($method === 'POST') {
    $input = json_decode(file_get_contents('php://input'), true) ?: [];
    $action = $input['action'] ?? '';

    // Öffentlich: Code einlösen (von Bestellformular)
    if ($action === 'redeem') {
    ...
    if ($action === 'save') {
        ...
        // Bestehende Werte als Fallback bei Update
        $found = false;
        foreach ($codes as &$c) {
    ...
    sp_json_response(['error' => 'Unbekannte Aktion.'], 400);
}
